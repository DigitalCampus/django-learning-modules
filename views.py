...
def recent_activity(request,id):
    ...
    for i in range(31,-1,-1):
        ...
        count_act_page = Tracker.objects.filter(digest__in=page_activity_digests,submitted_date__day=day,submitted_date__month=month,submitted_date__year=year).count()
        count_act_quiz = Tracker.objects.filter(digest__in=quiz_activity_digests,submitted_date__day=day,submitted_date__month=month,submitted_date__year=year).count()
        count_media = Tracker.objects.filter(digest__in=media_digests,submitted_date__day=day,submitted_date__month=month,submitted_date__year=year).count()
...
def recent_activity_detail(request,id):
    ...
    trackers = Tracker.objects.filter(Q(digest__in=quiz_activity_digests)|Q(digest__in=page_activity_digests)|Q(digest__in=media_digests)).order_by('-submitted_date')
